...
import {
  LayoutDashboard, QrCode, Barcode, Layers, Clock,
  Settings, Shield, Info, HelpCircle,
  ScanLine, ChevronRight, type LucideIcon,
} from "lucide-react";
...
const NAV: { label: string; to: string; icon: LucideIcon }[] = [
  ...
  { label: "Batch",       to: "/batch",   icon: Layers },
    ...
    >
      <div className="flex items-center gap-3 px-4 py-4 border-b" style={{ borderColor: "#2d3748", minHeight: 60 }}>
        <div
          ...
          style={{ width: 32, height: 32, background: "linear-gradient(135deg,#10b981,#059669)" }}
        >
          <ScanLine size={16} color="white" strokeWidth={2.5} />
        </div>
        <div>
          <div className="text-sm font-semibold leading-tight truncate" style={{ color: "#f1f5f9", whiteSpace: "nowrap" }}>QR Barcode Studio</div>
