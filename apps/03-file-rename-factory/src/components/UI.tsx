...
const VARIANT_STYLE: Record<BtnVariant, React.CSSProperties> = {
  primary:   { background: "#f97316", color: "#fff", border: "1px solid transparent" },
    ...
    <Card>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-2xl font-bold tracking-tight" style={{ color: accent ? "#f97316" : "#f1f5f9" }}>
            {value}
          </p>
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 px-8 text-center animate-fade-in">
      {illustration ? (
        ...
        <div className="rounded-2xl p-4 mb-4" style={{ background: "rgba(249,115,22,.08)", color: "#f97316" }}>
  ...
  return (
    <div className="flex items-start justify-between gap-6">
      ...
      <button
        ...
        style={{ width: 40, height: 22, background: checked ? "#f97316" : "#2d3748", position: "relative" }}
...
export function Switch({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)}
      ...
      style={{ width: 32, height: 18, background: checked ? "#f97316" : "#2d3748", position: "relative" }}
      role="switch" aria-checked={checked}>
