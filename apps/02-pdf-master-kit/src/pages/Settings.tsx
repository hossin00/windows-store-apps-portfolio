    ...
    <div className="flex flex-col h-full" style={{ background: "#0f1117" }}>
      ...
      <div className="flex-1 overflow-y-auto p-6 max-w-xl space-y-5">
            ...
            <div className="flex gap-2">
              {([["dark","Dark",Moon],["light","Light",Sun],["system","System",Monitor]] as [Theme, string, typeof Moon][]).map(([v, l, Icon]) => (
                <button key={v} onClick={() => updateSettings({ theme: v })}
                  ...
                  style={{ background: settings.theme === v ? "rgba(139,92,246,.12)" : "#1e2535",
                    borderColor: settings.theme === v ? "#8b5cf6" : "#2d3748",
        ...
        <SettingsCard title="Output">
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium" style={{ color: "#94a3b8" }}>Default filename prefix</label>
            <input value={settings.defaultFilenamePrefix}
              onChange={(e) => updateSettings({ defaultFilenamePrefix: e.target.value })}
              className="rounded-md px-3 py-2 text-sm outline-none"
              style={{ background: "#1e2535", border: "1px solid #2d3748", color: "#f1f5f9", maxWidth: 240 }} />
            <p className="text-xs mt-1" style={{ color: "#475569" }}>
              Output files are named with this prefix plus operation type and timestamp.
            </p>
          </div>
          <Divider />
          <ToggleRow label="Auto-download results"
            description="When enabled, completed PDFs are downloaded automatically. Otherwise they will appear in history only."
            checked={settings.autoDownload} onChange={(v) => updateSettings({ autoDownload: v })} />
        </SettingsCard>

        <SettingsCard title="History">
          <ToggleRow label="Save operation history locally"
            description="Keep a local record of every operation. You can clear this any time."
            checked={settings.saveHistory} onChange={(v) => updateSettings({ saveHistory: v })} />
        </SettingsCard>

        <SettingsCard title="Privacy">
          <ToggleRow label="Privacy mode"
            description="When enabled, operation history is not saved and any in-memory results are cleared after each operation."
        ...
        <SettingsCard title="Local Data">
          <div className="flex items-center justify-between gap-4 mb-4">
            ...
            <span className="text-sm font-semibold" style={{ color: "#8b5cf6" }}>{storageKB} KB</span>
          </div>
          ...
          <p className="text-xs mt-2" style={{ color: "#475569" }}>
            Removes operation history and settings stored locally. Original and output PDFs you have downloaded are unaffected.
          </p>
        </SettingsCard>
      </div>
    </div>
