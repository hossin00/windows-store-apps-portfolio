...
import {
  LayoutDashboard, Combine, Scissors, Minimize2, RotateCw, ArrowUpDown, FileSearch,
  Clock, Settings, Shield, Info, HelpCircle,
  FileStack, ChevronRight, type LucideIcon,
} from "lucide-react";

const NAV: { label: string; to: string; icon: LucideIcon }[] = [
  { label: "Dashboard", to: "/",         icon: LayoutDashboard },
  { label: "Merge",     to: "/merge",    icon: Combine         },
  { label: "Split",     to: "/split",    icon: Scissors        },
  { label: "Compress",  to: "/compress", icon: Minimize2       },
  { label: "Rotate",    to: "/rotate",   icon: RotateCw        },
  { label: "Reorder",   to: "/reorder",  icon: ArrowUpDown     },
  { label: "Extract",   to: "/extract",  icon: FileSearch      },
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 px-4 py-4 border-b" style={{ borderColor: "#2d3748", minHeight: 60 }}>
        ...
        >
          <FileStack size={16} color="white" strokeWidth={2.5} />
        </div>
        <div>
          <div className="text-sm font-semibold leading-tight truncate" style={{ color: "#f1f5f9", whiteSpace: "nowrap" }}>PDF Master Kit</div>
