...
// Accent palette for this app: red-500
export const ACCENT      = "#ef4444";
export const ACCENT_SOFT = "#f87171";
export const ACCENT_DEEP = "#b91c1c";
...
const VARIANT_STYLE: Record<BtnVariant, React.CSSProperties> = {
  primary:   { background: "#ef4444", color: "#fff", border: "1px solid transparent" },
    ...
    <Card>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-2xl font-bold tracking-tight" style={{ color: accent ? "#ef4444" : "#f1f5f9" }}>
            {value}
          </p>
        </div>
        <div className="rounded-lg p-2 flex-shrink-0" style={{ background: "rgba(239,68,68,.1)", color: "#ef4444" }}>
          {icon}
        </div>
      </div>
    </Card>
...
// ─── StatusBadge ──────────────────────────────────────────────────────────────
type ScanStatus = "waiting" | "scanning" | "complete" | "failed";
const STATUS_CFG: Record<ScanStatus, { color: string; label: string; pulse: boolean }> = {
  waiting:  { color: "#64748b", label: "Waiting",  pulse: false },
  scanning: { color: "#f87171", label: "Scanning", pulse: true  },
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs font-medium">
      <span
        ...
        style={{ display: "inline-block", width: 7, height: 7, borderRadius: "50%", background: cfg.color, flexShrink: 0 }}
      />
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 px-8 text-center animate-fade-in">
      {illustration ? (
        ...
        <div className="rounded-2xl p-4 mb-4" style={{ background: "rgba(239,68,68,.08)", color: "#ef4444" }}>
  ...
  return (
    <div className="flex items-start justify-between gap-6">
      ...
      <button
        ...
        style={{ width: 40, height: 22, background: checked ? "#ef4444" : "#2d3748", position: "relative" }}
...
// ─── TextField ────────────────────────────────────────────────────────────────
export function TextField({ label, value, onChange, placeholder, type = "text", multiline, rows = 3, mono, min, max, step, suffix }: {
      ...
      {multiline ? (
        ...
        <div className="flex items-stretch">
          <input type={type} value={value} placeholder={placeholder}
            ...
            style={{ ...shared, borderRight: suffix ? "none" : "1px solid #2d3748" }} />
          {suffix && (
    ...
    <div className="flex gap-2 flex-wrap">
      {options.map((o) => (
        <button key={o.value} onClick={() => onChange(o.value)}
          ...
          style={{
            background:  value === o.value ? "rgba(239,68,68,.12)" : "#1e2535",
            borderColor: value === o.value ? ACCENT : "#2d3748",
            color:       value === o.value ? "#f87171" : "#94a3b8",
          }}>{o.label}</button>
      ))}
    </div>
  );
