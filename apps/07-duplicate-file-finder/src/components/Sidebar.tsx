...
import {
  LayoutDashboard, Search as SearchIcon, BarChart3, Clock,
  Settings, Shield, Info, HelpCircle, Files,
  ChevronRight, type LucideIcon,
} from "lucide-react";

const NAV: { label: string; to: string; icon: LucideIcon }[] = [
  { label: "Dashboard", to: "/",         icon: LayoutDashboard },
  { label: "Scanner",   to: "/scanner",  icon: SearchIcon },
    ...
    >
      <div className="flex items-center gap-3 px-4 py-4 border-b" style={{ borderColor: "#2d3748", minHeight: 60 }}>
        <div
          ...
          style={{ width: 32, height: 32, background: "linear-gradient(135deg,#ef4444,#b91c1c)" }}
        >
          <Files size={16} color="white" strokeWidth={2.5} />
        </div>
        <div>
          <div className="text-sm font-semibold leading-tight truncate" style={{ color: "#f1f5f9", whiteSpace: "nowrap" }}>Duplicate File Finder</div>
...
function SidebarLink({
  ...
  return (
    <NavLink
      ...
      style={{ color: active ? "#f1f5f9" : "#64748b", background: active ? "rgba(239,68,68,.12)" : "transparent" }}
    >
      <span style={{ color: active ? "#ef4444" : undefined, flexShrink: 0 }}>
        <Icon size={17} strokeWidth={active ? 2.2 : 1.8} />
      </span>
      <span className="flex-1 truncate" style={{ whiteSpace: "nowrap" }}>{label}</span>
      {active && <ChevronRight size={13} style={{ color: "#ef4444", opacity: 0.7 }} />}
    </NavLink>
  );
}
