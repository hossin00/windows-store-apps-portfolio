...
// Accent palette for this app: sky-500
export const ACCENT      = "#0ea5e9";
export const ACCENT_SOFT = "#38bdf8";
export const ACCENT_DEEP = "#0369a1";
...
const VARIANT_STYLE: Record<BtnVariant, React.CSSProperties> = {
  primary:   { background: "#0ea5e9", color: "#fff", border: "1px solid transparent" },
    ...
    <Card>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-2xl font-bold tracking-tight" style={{ color: accent ? "#0ea5e9" : "#f1f5f9" }}>
            {value}
          </p>
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 px-8 text-center animate-fade-in">
      {illustration ? (
        ...
        <div className="rounded-2xl p-4 mb-4" style={{ background: "rgba(14,165,233,.08)", color: "#0ea5e9" }}>
  ...
  return (
    <div className="flex items-start justify-between gap-6">
      ...
      <button
        ...
        style={{ width: 40, height: 22, background: checked ? "#0ea5e9" : "#2d3748", position: "relative" }}
...
// ─── TextField ────────────────────────────────────────────────────────────────
export function TextField({ label, value, onChange, placeholder, type = "text", multiline, rows = 3, mono, min, max, step, suffix, prefix, maxLength }: {
  label?: string; value: string; onChange: (v: string) => void;
  placeholder?: string; type?: string; multiline?: boolean; rows?: number; mono?: boolean;
  min?: number; max?: number; step?: number; suffix?: string; prefix?: string; maxLength?: number;
}) {
    ...
    <div className="w-full">
      ...
      {multiline ? (
        ...
        <div className="flex items-stretch">
          {prefix && (
            <span className="flex items-center px-2 text-xs rounded-l-md"
              style={{ background: "#161b27", border: "1px solid #2d3748", borderRight: 0, color: "#64748b" }}>
              {prefix}
            </span>
          )}
          <input type={type} value={value} placeholder={placeholder}
            ...
            onChange={(e) => onChange(e.target.value)}
            className={`w-full px-3 py-2 text-sm outline-none ${prefix ? "" : "rounded-l-md"} ${suffix ? "" : "rounded-r-md"}`}
            style={{ ...shared, borderLeft: prefix ? "none" : "1px solid #2d3748", borderRight: suffix ? "none" : "1px solid #2d3748" }} />
          {suffix && (
    ...
    <div className="flex gap-2 flex-wrap">
      {options.map((o) => (
        <button key={o.value} onClick={() => onChange(o.value)}
          ...
          style={{
            background:  value === o.value ? "rgba(14,165,233,.12)" : "#1e2535",
            borderColor: value === o.value ? ACCENT : "#2d3748",
            color:       value === o.value ? "#38bdf8" : "#94a3b8",
          }}>{o.label}</button>
      ))}
    </div>
  );
